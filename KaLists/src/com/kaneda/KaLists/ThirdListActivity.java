...
public class ThirdListActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_ichiran_list);

		ListView lvTemple = (ListView) findViewById(R.id.lvn_temple);
	...
	private  List<String> createPrefectureList(){
				...
				if(extras != null){
					resultsString = extras.getString("selectedgroup");
				}
				...
				String sql = "SELECT * FROM player WHERE name Like = '*" +resultsString + "*'";
				...
				try{
					...
					while(cursor != null && cursor.moveToNext()){
						...
						//球団名の生成
						int idexContent = cursor.getColumnIndex("pgroup");
						pgroup = cursor.getString(idexContent);

						//選手名の生成
						idexContent = cursor.getColumnIndex("name");
						name = cursor.getString(idexContent);
						//リストに入力する
						playerList.add(pgroup+"　"+name);
